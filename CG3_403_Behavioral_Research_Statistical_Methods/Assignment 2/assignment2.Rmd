...
if (file.exists(mosquito_file)) {
  mosq <- read.csv(mosquito_file)
  cat("Columns found:", paste(colnames(mosq), collapse=", "), "
")
  if (all(c("Beer", "Water") %in% colnames(mosq))) {
    beer  <- na.omit(mosq$Beer)
    water <- na.omit(mosq$Water)
  } else {
    grp   <- mosq[[1]]
    count <- mosq[[2]]
    beer  <- count[grp == unique(grp)[1]]
    water <- count[grp == unique(grp)[2]]
  }
} else {
